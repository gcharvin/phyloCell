...
dMat(dMat~=dMat)=Inf; % get rid of NaNs

% #todo: try enabling log compression here
% dMat = log(dMat);
maxcost=max(dMat(dMat<Inf))*dim+1;
if isempty(maxcost)
    maxcost = Inf;
end
dMat(dMat==Inf)=maxcost; % get rid of infs

%% begin modified code
% moved here by pmm to make sure resolution is finite (not NaN)
if nargin<2
    resolution=eps(max(max(dMat)));
end
% end modified code
%%
...
% dMat = dMat(1:rdim,1:cdim);
% dMat = dMat - u(:,ones(1,cdim)) - v(ones(rdim,1),:);
% adding this part to make output identical to Munkres (corrected 2012-06-13 MLT)
assignment = false(rdim,cdim);
for rowind=1:rdim
    assignment(rowind,rowsol(rowind)) = true;
end
if swapf
