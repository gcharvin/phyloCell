function [xx yy imdistance]=phy_segmentPombe(imag)
%function B=phy_segmentPombe(frame,display,cellcelldistance)
% returns the skeleton of Pombe cells


%imag=loadTimeLapseImage(2,frame,1,' ');
display=0;
cellcelldistance=25;
[xx yy imdistance]=findCellCenters(imag,display,cellcelldistance);


function [xx yy imdistance2]=findCellCenters(imdata,display,cellcelldistance)
...
sizemin=cellcelldistance/1.5; % minimal distance to edge to be considered, roughly the min cell radius , pixel units
sizemax=cellcelldistance*2; % maximal distance to edge that is tolerable
...
C=imdilate(C,strel('disk',round(cellcelldistance/8)));
