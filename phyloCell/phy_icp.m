...
elseif or(nargin==1,nargin==2)
    ...
    if bol
        ...
        if (size(MODEL,2)==size(MODEL,1))
            ...
            %comented by andrei
            warning('This icp method demands the number of model points to be greater then the dimension.');%added by andrei
            tes_flag=3;%added by andrei
        end
...
global MODEL DATA TR TT iclosest ERROR distance %iclosest, ERROR distance added by andrei
distance=[];
ERROR=0;
iclosest=[];
TT=[];
...
if isempty(model)
    %error('Model can not be an empty matrix.');
    %comented by andrei
    warning('Model can not be an empty matrix.'); % added by andrei
    return      %added by andrei
end

%model

if (size(model,2)<size(model,1)) && (min(size(model))>1) % && added by andrei
    MODEL=model';
else
    MODEL=model;
end

if (size(data,2)<size(data,1)) && (min(size(data))>1) % && added by andrei
    data=data';
    DATA=data;
else
    DATA=data;
...
if size(DATA,1)~=size(MODEL,1)
    ...
    %comented by andrei
    warning('Different dimensions of DATA and MODEL!'); %added by andrei
    return      %added by andrei
end
...
elseif fitting(1)==3
    ...
    elseif fitting(2)==1 %added by andrei
    elseif fitting(2)>1
...
for iter=1:max_iter
    ...
    if iter<min_iter
        indexUsed=icp_transformation(fitting,[]);         % find transformation
    else
        indexUsed=icp_transformation(fitting,refpnt);     % find transformation
    end
...
distance = 0;  
for i=1:N
            distance(i)=norm(MODEL(:,iclosest(i))-DATA(:,i));
end

ERROR=0;
ERROR=sum(distance(indexUsed));
...
elseif init_flag==1
    ...
    if fitting(1)==2
        ...
        if length(fitting)==1
           % mem=mean(MODEL,2); med=mean(DATA,2); %comented by andrei
            mem=median(MODEL,2); med=median(DATA,2); % added by andrei
        else
...
if tes_flag==3
    ...
    global MODEL DATA iclosest distance
    md=size(DATA,2);
    mm=size(MODEL,2);

    iclosest=zeros(1,md);
    distance=zeros(1,md); %added by andrei
    ...
    for id=1:md
        ...
        for im=1:mm
            ...
            if dista<dist
                iclosest(id)=im;
                dist=dista;
                distance(id)=dista; %added by andrei
            end
        end
...
elseif tes_flag==1
    ...
    global MODEL DATA Tes ir jc iclosest distance
        ...
        for in=ir(jc(iclosest(mid)):(jc(iclosest(mid)+1)-1))
            for ind=Tes(in,:)
                ...
                if distcc<distc
                    ...
                    bol=not(bol);
                    iclosest(mid)=ind;
                    distance(mid)=distcc; %added by andrei
                    break;
                end
            end
            if bol
    ...
    for id = (mid+1):md
       ...
       distance(id)=distance(id-1); %added by andrei
        bol=not(bol);
        while bol
            bol=not(bol);
            ...
            for in=ir(jc(iclosest(id)):(jc(iclosest(id)+1)-1))
                for ind=Tes(in,:)
                    ...
                    if distcc<distc
                        ...
                        bol=not(bol);
                        iclosest(id)=ind;
                        distance(id)=distcc; %added by andrei
                        break;
                    end
                end
                if bol
    ...
    global MODEL DATA Tes Tesind Tesver icTesind iclosest distance
    ...
    distance(mid)=so(1); %added by andrei
...
elseif tes_flag==1
    ...
    global MODEL DATA Tes Tesind Tesver iclosest icTesind distance
...
function ind=icp_transformation(fitting,refpnt) % "ind=" added by andrei
% Finds the rotation and translation of the DATA points

global MODEL DATA iclosest TR TT distance
...
if bol
    ...
    elseif fitting(1)==3
        ...
        %modified by andrei
        %figure; plot(V);
        % remove the repeted values in iclosest
        
%             repetedVal=[];
%             for i=1:length(ind)
%                 for j=1:i-1
%                     if iclosest(ind(i))==iclosest(ind(j))
%                         repetedVal=[repetedVal i];
%                     end
%                 end
%             end
%             ind(repetedVal)=[];
        
        %remove the outliers from the clouds points
        V2=V(ind);
        median_distance=median(V2);
        std_distance=std(V2);
        ind2=find(V2<=(median_distance+1.5*std_distance));
        fitting(2)=length(ind2);
        ind=ind(ind2);
        %andrei
...
elseif fitting(1)==3
    ERR=dist^2;%commented by andrei
    %ERR=dist; %added by andrei
else
    ERR=0;
