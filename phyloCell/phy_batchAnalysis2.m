function phy_batchAnalysis(index,option,channel)
...
for indez=index
    ...
    cellsindices=[ncell segmentation.tcells1(ncell).daughterList];
    
    iarr=zeros(length(cellsindices),timeLapse.numberOfFrames);
    
    cc=1;
    for i=cellsindices
        temp=[segmentation.tcells1(i).Obj.image];
        im=[im temp];
...
%     end
    ...
    for i=im
        ...
        if option==0 % cytosplasmic fluorescence scoring
            segmentation=measureFluorescence(cellsindices,imgarr,iarr,segmentation,i);
        end
...
function segmentation=measureFluorescence(cells,imgarr,segmentation)
