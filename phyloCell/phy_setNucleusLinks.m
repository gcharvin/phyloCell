function mothers=phy_setNucleusLinks(object,noiteration)
...
%if numel(candarrstore)==0
    ...
    for k=1:numel(narr(:,1))
      ...
      %  else
       ...
       % return;
        %end
        % rule 2 : size ratio between mother and daughter should be larger than 2;
...
if nargin==2 % no iterations wanted : just build the pedigree tree based on scoring
    ...
    while cc<10*length(narr(:,3))
        ...
        swap=randi(size(problems,1));
        problems=problems(swap,:);
        ...
        %if numel(problems)==0
            ...
            if candarr(pix,1)~=0
  %'ok'
           problems(2:end+1,:)= problems(1:end,:);
    ...
    if numel(ener)~=0
        ...
        fprintf(['Could not assign some of the daughter cells:' num2str([problems(:,2) problems(:,3)]) ' -exiting....\n']);
        %problems
        %mothers=[];
        %return;
    end
