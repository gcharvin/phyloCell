...
function [budneck]=phy_segmentEvi(img,parametres)
...
warning off all
background = imopen(img,strel('disk',3));
warning on all
...
for cc = 1:length(B)
   ...
   % 'ok'
    ...
    if min(boundary(:,2))>0 && max(min(boundary(:,2)))<size(img,2) && min(boundary(:,1))>0 && max(min(boundary(:,1)))<size(img,1)
