function [hf,h,imgout]=phy_showImage_3bars(varargin)
...
imgout=insertText(imgin,[xpos ypos],timestamp,'Font','Arial Bold','FontSize',30,'BoxColor',...
...
imgout=imgin;
keyFrames=str2num(sequence.frames);
f=numel(keyFrames);
...
maxKeyFrames=max(keyFrames)-1;%-min(keyFrames);

pix = find(keyFrames<i,1,'last'); %indice of current keyframe interval

prog(1:keyFrames(end))=[0:keyFrames(end)-1]./(keyFrames(end)-1);%to later replace keyFrames(end) with numel(frames) to be more versatile // -1 because frames start at t=0
% elseif pix(end)>=1 %the last value is the good one
% prog(i)=i/keyFrames(end);

xpos(1)=prog(keyFrames(1))*0.5*ROI(3)   +10;
for m=2:f
    xpos(m)=(prog(keyFrames(m))+prog(keyFrames(m-1)))*0.5*ROI(3)   +10;
end
ypos=1*ROI(4)-40;


KeyFramesStr=strfind(sequence.str,'*');
KeyFramesStr=[KeyFramesStr length(sequence.str)];

%timestamp='';

for m=1:f
    keyFrames_Label{m}=sequence.str(KeyFramesStr(m)+1:KeyFramesStr(m+1)-1);
end
if numel(pix)==0
    pix2=1; %Theo add pour avoir un xpos qui depend de où st i par rapport au keyframes
    %timestamp= sequence.str(KeyFramesStr(pix2)+1:KeyFramesStr(pix2+1)-1); %recup les lettres entre 2 étoiles
end

if numel(pix)>0
    pix2=pix+1; %Theo add
%     if pix>=numel(KeyFramesStr)
%         timestamp='';
%     else
        %timestamp= sequence.str(KeyFramesStr(pix2)+1:KeyFramesStr(pix2+1)-1); %recup les lettres entre 2 étoiles
%     end
...
if figureoutput==1 % do not display in movies
text(xpos,ypos,keyFrames_Label,'Color','w','FontSize',20); %'FontWeight','bold',
else

imgout = insertShape(imgout,'Rectangle',[10 ROI(4)-20 (ROI(3)-20) 10],'Color', 'w','Opacity',1,'LineWidth',2);

if 0<i && i<=keyFrames(1)
imgout = insertShape(imgout,'FilledRectangle',[10 ROI(4)-20 prog(i)*(ROI(3)-20) 10],'Color', 'w','Opacity',1,'LineWidth',2);

imgout=insertText(imgout,[xpos(pix2) ypos],keyFrames_Label{pix2},'Font','Arial','FontSize',30,'BoxColor',...
    [1 1 1],'BoxOpacity',0.0,'TextColor','white','AnchorPoint','Center');

elseif keyFrames(1)<i && i<=keyFrames(2)
imgout = insertShape(imgout,'FilledRectangle',[10      ROI(4)-20      prog(keyFrames(1))*(ROI(3)-20)       10],'Color', 'w','Opacity',1,'LineWidth',2);
imgout = insertShape(imgout,'FilledRectangle',[10+prog(keyFrames(1))*(ROI(3)-20)      ROI(4)-20     (prog(i)-prog(keyFrames(1)))*(ROI(3)-20)      10],'Color', [163 163 163],'Opacity',1,'LineWidth',2);

imgout=insertText(imgout,[xpos(pix2-1) ypos],keyFrames_Label{pix2-1},'Font','Arial Bold','FontSize',30,'BoxColor',...
    [1 1 1],'BoxOpacity',0.0,'TextColor','white','AnchorPoint','Center');
imgout=insertText(imgout,[xpos(pix2) ypos],keyFrames_Label{pix2},'Font','Arial Bold','FontSize',30,'BoxColor',...
    [1 1 1],'BoxOpacity',0.0,'TextColor','white','AnchorPoint','Center');

elseif keyFrames(2)<i && i<=keyFrames(3)
imgout = insertShape(imgout,'FilledRectangle',[10      ROI(4)-20    prog(keyFrames(1))*(ROI(3)-20) 10],'Color', 'w','Opacity',1,'LineWidth',2);
imgout = insertShape(imgout,'FilledRectangle',[10+prog(keyFrames(1))*(ROI(3)-20)       ROI(4)-20       (prog(keyFrames(2))-prog(keyFrames(1)))*(ROI(3)-20)       10],'Color', [163 163 163],'Opacity',1,'LineWidth',2);
imgout = insertShape(imgout,'FilledRectangle',[10+prog(keyFrames(2))*(ROI(3)-20)         ROI(4)-20         (prog(i)-prog(keyFrames(2)))*(ROI(3)-20)       10],'Color', 'w','Opacity',1,'LineWidth',2);

imgout=insertText(imgout,[xpos(pix2-2) ypos],keyFrames_Label{pix2-2},'Font','Arial Bold','FontSize',30,'BoxColor',...
    [1 1 1],'BoxOpacity',0.0,'TextColor','white','AnchorPoint','Center');
imgout=insertText(imgout,[xpos(pix2-1) ypos],keyFrames_Label{pix2-1},'Font','Arial Bold','FontSize',30,'BoxColor',...
    [1 1 1],'BoxOpacity',0.0,'TextColor','white','AnchorPoint','Center');
imgout=insertText(imgout,[xpos(pix2) ypos],keyFrames_Label{pix2},'Font','Arial Bold','FontSize',30,'BoxColor',...
...
% elseif keyFrames(3)<i && i<=keyFrames(4)
% imgout = insertShape(imgout,'FilledRectangle',[10                        ROI(4)-20                      prog(keyFrames(1))*(ROI(3)-20)           10],'Color', 'w','Opacity',1,'LineWidth',2);
% imgout = insertShape(imgout,'FilledRectangle',[10+prog(keyFrames(1))*(ROI(3)-20)       ROI(4)-20           (prog(keyFrames(2))-prog(keyFrames(1)))*(ROI(3)-20)       10],'Color', [163 163 163],'Opacity',1,'LineWidth',2);
% imgout = insertShape(imgout,'FilledRectangle',[10+prog(keyFrames(2))*(ROI(3)-20)         ROI(4)-20         (prog(keyFrames(3))-prog(keyFrames(2)))*(ROI(3)-20)       10],'Color', [254 235 207],'Opacity',1,'LineWidth',2);
% imgout = insertShape(imgout,'FilledRectangle',[10+prog(keyFrames(3))*(ROI(3)-20)         ROI(4)-20         (prog(i)-prog(keyFrames(3)))*(ROI(3)-20)       10],'Color', [255 247 237],'Opacity',1,'LineWidth',2);
% 
% imgout=insertText(imgout,[xpos(pix2-3) ypos],keyFrames_Label{pix2-3},'Font','Arial Bold','FontSize',30,'BoxColor',...
%     [1 1 1],'BoxOpacity',0.0,'TextColor','white','AnchorPoint','Center');
% imgout=insertText(imgout,[xpos(pix2-2) ypos],keyFrames_Label{pix2-2},'Font','Arial Bold','FontSize',30,'BoxColor',...
%     [1 1 1],'BoxOpacity',0.0,'TextColor','white','AnchorPoint','Center');
% imgout=insertText(imgout,[xpos(pix2-1) ypos],keyFrames_Label{pix2-1},'Font','Arial Bold','FontSize',30,'BoxColor',...
...
%delimiting bars
for k=1:numel(keyFrames)-1
    
    xstart=(keyFrames(k)-1)./maxKeyFrames;%-min(keyFrames))./maxKeyFrames; 

    xstart=xstart*(ROI(3)-20);

    imgout = insertShape(imgout,'Line',[10+xstart ROI(4)-20 10+xstart ROI(4)-10],'Color', 'black','Opacity',1,'LineWidth',2);   
end
